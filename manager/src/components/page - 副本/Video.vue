           ...
           <el-table-column
			...
			show-overflow-tooltip>
			    <template    slot-scope="scope">            
			        <div title="点击放大播放" >
			            <video controls="controls" style="width:60px; height:20px" :src="qiniu.showUrl+scope.row.vdoName"></video>
			            
			            <el-popover
			            placement="right"
			            trigger="click"
			            >
			                <video controls="controls" style="max-height:400px" :src="qiniu.showUrl+scope.row.vdoName"></video>
...
<!--                <el-button
        ...
        <!-- 弹窗2 -->
            ...
            style="margin-left:3%;margin-right:3%;margin-bottom:10px"
                ...
                :auto-upload="false">
                <el-button slot="trigger" size="small" type="primary" style="margin-bottom:10px" >选取视频</el-button>
            ...
            <div style="width:94%; margin:20px auto">
                ...
                <el-table
                    ...
                    show-overflow-tooltip>
                        <template    slot-scope="scope">            
                            <div title="点击放大播放"  >
                                <img :src="qiniu.showUrl+scope.row.vdoName"    style="width:60px; height:20px" />
                                <el-popover
                                placement="right"
                                trigger="click"
                                >
                                    <img :src="qiniu.showUrl+scope.row.vdoName"    style="max-height:400px" />
                                    <i slot="reference"  class="el-icon-view" style="font-size:16px"></i>
                                </el-popover>
                            </div>
                        </template>  
...
export default {
    ...
    methods:{
        ...
        updateDetailSuccess2(response, file, fileList){
            ...
            if(this.uploadNum2==this.imgListLength2){
                ...
                for(let i=0;i<fileList.length;i++){
                    ...
                    let obj={"vdoName":"","vslDpyId":this.table2pmas.vslDpyId}
                    obj.vdoName=fileList[i].response.key
                ...
                if(this.submitTag2=="change"){
                    ...
                    this.detailsUpPamas.vdoName=fileKeys[0]
        ...
        changeDetail(index, rows){
            ...
            this.upfileList2=[{url:this.qiniu.showUrl+rows[index].vdoName}]
            
            this.detailsUpPamas={
                vdoName:rows[index].vdoName,
                id:rows[index].id,
                oldVdoName:rows[index].vdoName
            }
        },
        ...
        console.log("deleteRow",rows[index].id,rows[index].vdoName)
        ...
        deletAll2(){
            ...
            for(let i=0;i<this.multipleSelection2.length;i++){
                ...
                arrPictures.push(this.multipleSelection2[i].vdoName)
        ...
        updateDetailSuccess(response, file, fileList){
            ...
            if(this.uploadNum==this.imgListLength){
                ...
                if(this.submitTag=="change"){
                    this.oldPamas.vdoName=fileKeys[0]
        ...
        // 新增一条的接口
        addServer(pamas){
            postRequest(url.cmaVdoInsert, pamas).then(data=> {
                this.uploadNum=0
        ...
        // 更新一条的接口
        upServer(pamas){
            postRequest(url.cmaVdoUpdate, pamas).then(data=> {
                this.uploadNum=0
                this.dialogVisible=false
                console.log("上传到服务器成功返回",data)
                if(!getErr(data.data.code,data.data.message)) {
        ...
        changeRow(index, rows){
            ...
            this.upfileList=[{url:this.qiniu.showUrl+rows[index].vdoName}]
        ...
        deletMost(){
            ...
            for(let i=0;i<this.multipleSelection.length;i++){
                ...
                arrPictures.push(this.multipleSelection[i].vdoName)
        ...
        initData(pamas){
            postRequest(url.cmaVdoGetCmaVdoList, pamas).then(data=> {

...
export default {
    ...
    methods:{
        ...
        showDetail(index, rows){
            ...
            this.$router.push({ path: '/soft_detail' })
            localStorage.setItem("soft_id",rows[index].id)
        ...
        // 新增一条的接口
        addServer(pamas){
            postRequest(url.integralSoftPackingInsertIntegralSoftPacking, pamas).then(data=> {
                this.uploadNum=0
        ...
        // 更新一条的接口
        upServer(pamas){
            postRequest(url.integralSoftPackingUpdateIntegralSoftPackingById, pamas).then(data=> {
                this.uploadNum=0
                this.dialogVisible=false
                console.log("上传到服务器成功返回",data)
                if(!getErr(data.data.code,data.data.message)) {
        ...
        this.$confirm('是否删除?', '提示', {
            ...
            type: 'warning'
                ...
                postRequest(url.integralSoftPackingDeleteIntegralSoftPackingByArray, {
                    ids:rows[index].id,
                    urls:rows[index].url,
                }).then(data=> {
...
/***********************富文本的操作*****************************/ 
        ...
        subTextForm(){
            ...
            if(this.ifTextImg==false){
                ...
                this.addTextImg({
                    integralSoftPackingId:this.textImgId,
                    imgTex:this.content
                })
            }
        ...
        // 新增一条富文本的接口
        addTextImg(pamas){
            postRequest(url.integralSoftPackingDetailsInsertIntegralSoftPackingDetails, pamas).then(data=> {
        ...
        upTextImg(pamas){
            ...
            postRequest(url.integralSoftPackingDetailsUpdateIntegralSoftPackingDetailsById, pamas).then(data=> {
                if(!getErr(data.data.code,data.data.message)) {
                    return
                }
                this.uploadNum=0
        ...
        initTextImg(id){
            postRequest(url.integralSoftPackingDetailsSelectByIntegralSoftPackingId, {
                integralSoftPackingId:id
            }).then(data=> {
        ...
        initData(pamas){
            postRequest(url.integralSoftPackingSelectIntegralSoftPackingList, pamas).then(data=> {
    ...
    created(){
        ...
        for( let i=0;i<ms_dictionary.length;i++){
            if(ms_dictionary[i].key=='INTEGRAL_SOFT_PACKING_TYPE'){

...
export default {
    ...
    methods:{
        ...
        showDetail(index, rows){
            ...
            localStorage.setItem("case_id",rows[index].id)
        ...
        // 新增一条的接口
        addServer(pamas){
            postRequest(url.boutiqueCaseInsertBoutiqueCase, pamas).then(data=> {
                this.uploadNum=0
        ...
        // 更新一条的接口
        upServer(pamas){
            postRequest(url.boutiqueCaseUpdateBoutiqueCaseById, pamas).then(data=> {
                this.uploadNum=0
                this.dialogVisible=false
                console.log("上传到服务器成功返回",data)
                if(!getErr(data.data.code,data.data.message)) {
        ...
        this.$confirm('是否删除?', '提示', {
            ...
            type: 'warning'
                ...
                postRequest(url.boutiqueCaseDeleteBoutiqueCaseByArray, {
                    ids:rows[index].id,
                    urls:rows[index].url,
                }).then(data=> {
        ...
        deletMost(){
            ...
            console.log('请选择后删除',arrPictures)
...
/***********************富文本的操作*****************************/ 
        ...
        initTextImg(id){
            postRequest(url.boutiqueCaseDetailsSelectByBoutiqueCaseId, {
                boutiqueCaseId:id
            }).then(data=> {
        ...
        initData(pamas){
            postRequest(url.boutiqueCaseSelectBoutiqueCaseList, pamas).then(data=> {
    ...
    created(){
        ...
        for( let i=0;i<ms_dictionary.length;i++){
            if(ms_dictionary[i].key=='BOUTIQUE_CASE_TYPE'){
                this.typeList=ms_dictionary[i].dictionary
                console.log("当前的字典值", this.typeList)

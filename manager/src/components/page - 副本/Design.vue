...
export default {
    ...
    methods:{
        ...
        updateDetailSuccess2(response, file, fileList){
            ...
            if(this.uploadNum2==this.imgListLength2){
                ...
                for(let i=0;i<fileList.length;i++){
                    ...
                    let obj={"imgName":"","wdgDinId":this.table2pmas.wdgDinId}
                ...
                if(this.submitTag2=="add"){
                    ...
                    this.addServer2({
                        wdgDinDtsList:JSON.stringify(wdgIesDtsList),
                    })
                }
        ...
        addServer2(pamas){
            ...
            postRequest(url.wdgDinDtsInsert, pamas).then(data=> {
                this.uploadNum2=0
                this.submitTag2="add"
                this.dialogVisible=false
                
                console.log("上传到服务器成功返回22222",this.table2pmas)
                this.upfileList2=[]
        ...
        // 更新一条的接口
        upServer2(pamas){
            postRequest(url.wdgDinDtsUpdate, pamas).then(data=> {
                this.uploadNum2=0
                this.submitTag2="add"
        ...
        this.$confirm('是否删除?', '提示', {
            ...
            type: 'warning'
                ...
                postRequest(url.wdgDinDtsDelete, {
                    ids:rows[index].id,
                    imgName:rows[index].imgName,
                }).then(data=> {
        ...
        // 新增一条的接口
        addServer(pamas){
            postRequest(url.wdgDinInsert, pamas).then(data=> {
                this.uploadNum=0
        ...
        // 更新一条的接口
        upServer(pamas){
            postRequest(url.wdgDinUpdate, pamas).then(data=> {
                this.uploadNum=0
                this.dialogVisible=false
                console.log("上传到服务器成功返回",data)
                if(!getErr(data.data.code,data.data.message)) {
        ...
        initData(pamas){
            postRequest(url.wdgDinGetWdgDinList, pamas).then(data=> {
        ...
        initData2(pamas){
            postRequest(url.wdgDinDtsGetWdgDinDtsList, pamas).then(data=> {

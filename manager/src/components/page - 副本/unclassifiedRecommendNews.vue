        ...
        <el-table
           ...
           >
            <template slot-scope="scope">
                ...
                <!-- <el-button
                @click.native.prevent="newsDetail(scope.$index, tableList)"
                type="success"
                size="small">
                详情
                </el-button> -->
...
export default {
    ...
    methods:{
        ...
        newsDetail(index, rows){
            this.$router.push({ path: '/news_detail' })
            localStorage.setItem("news_id",rows[index].id)
        },
        ...
        // 新增一条的接口
        addServer(pamas){
            postRequest(url.recommendedNewsInsertRecommendedNews, pamas).then(data=> {
                this.uploadNum=0
        ...
        // 更新一条的接口
        upServer(pamas){
            postRequest(url.recommendedNewsUpdateRecommendedNewsById, pamas).then(data=> {
                this.uploadNum=0
        ...
        this.$confirm('是否删除?', '提示', {
            ...
            type: 'warning'
                ...
                postRequest(url.recommendedNewsDeleteRecommendedNewsByArray, {
                    ids:rows[index].id,
...
/***********************富文本的操作*****************************/ 
        ...
        subTextForm(){
            ...
            if(this.ifTextImg==false){
                ...
                this.addTextImg({
                    recommendedNewsId:this.textImgId,
                    imgTex:this.content
                })
            }
        ...
        // 新增一条富文本的接口
        addTextImg(pamas){
            postRequest(url.recommendedNewsDetailsInsertRecommendedNewsDetails, pamas).then(data=> {
        ...
        upTextImg(pamas){
            ...
            postRequest(url.recommendedNewsDetailsUpdateRecommendedNewsDetailsById, pamas).then(data=> {
                if(!getErr(data.data.code,data.data.message)) {
                    return
                }
                this.uploadNum=0
        ...
        initTextImg(id){
            postRequest(url.recommendedNewsDetailsSelectByRecommendedNewsId, {
                recommendedNewsId:id
            }).then(data=> {
        ...
        initData(pamas){
            postRequest(url.recommendedNewsSelectRecommendedNewsList, pamas).then(data=> {
                if(!getErr(data.data.code,data.data.message)) {
                    return
                }

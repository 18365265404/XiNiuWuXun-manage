...
export default {
    ...
    methods:{
        ...
        updateDetailSuccess2(response, file, fileList){
            ...
            if(this.uploadNum2==this.imgListLength2){
                ...
                for(let i=0;i<fileList.length;i++){
                    ...
                    let obj={"imgName":"","pngLotId":this.table2pmas.pngLotId}
                ...
                if(this.submitTag2=="add"){
                    ...
                    this.addServer2({
                        pngLotDtsList:JSON.stringify(wdgIesDtsList),
                    })
                }
        ...
        addServer2(pamas){
            ...
            postRequest(url.pngLotDtsInsert, pamas).then(data=> {
                this.uploadNum2=0
                this.submitTag2="add"
        ...
        // 更新一条的接口
        upServer2(pamas){
            postRequest(url.pngLotDtsUpdate, pamas).then(data=> {
                this.uploadNum2=0
                this.submitTag2="add"
        ...
        this.$confirm('是否删除?', '提示', {
            ...
            type: 'warning'
                ...
                postRequest(url.pngLotDtsDelete, {
                    ids:rows[index].id,
                    imgName:rows[index].imgName,
                }).then(data=> {
        ...
        // 新增一条的接口
        addServer(pamas){
            postRequest(url.pngLotInsert, pamas).then(data=> {
                this.uploadNum=0
        ...
        // 更新一条的接口
        upServer(pamas){
            postRequest(url.pngLotUpdate, pamas).then(data=> {
                this.uploadNum=0
                this.dialogVisible=false
                console.log("上传到服务器成功返回",data)
                if(!getErr(data.data.code,data.data.message)) {
        ...
        initData(pamas){
            postRequest(url.pngLotGetPngLotList, pamas).then(data=> {
        ...
        initData2(pamas){
            postRequest(url.pngLotDtsGetPngLotDtsList, pamas).then(data=> {

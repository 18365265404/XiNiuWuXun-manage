<template>
    <div>
        ...
        <div  class="button-add" style=" display:flex;  justify-content: space-between; overflow: hidden; margin-top:20px; margin-left:2%;">
            ...
            <el-button v-show="tableList.length!=0 ? false : true"  style=" margin-right:2%;margin-bottom:20px" type="primary"  @click="addMost()">创建</el-button>  
        </div>
...
export default {
    ...
    methods:{
        ...
        addServer(pamas){
            postRequest(url.integralSoftPackingDetailsInsertIntegralSoftPackingDetails, pamas).then(data=> {
                ...
                this.initData({
                    integralSoftPackingId:this.firstId
                })
        ...
        initData(pamas){
            postRequest(url.integralSoftPackingDetailsSelectByIntegralSoftPackingId, pamas).then(data=> {
    ...
    created(){
        let firstId=localStorage.getItem("soft_id")
        this.firstId=firstId
        this.initData({
            integralSoftPackingId:firstId
        })
        this.initQiniu()

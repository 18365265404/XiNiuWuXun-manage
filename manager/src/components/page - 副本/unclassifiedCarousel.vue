        ...
        <!-- 分页 -->
          <div class="block" style="overflow:hidden;display:none">
            ...
            style="margin-left:3%;margin-right:4%"
                ...
                :auto-upload="false">
                <el-button  slot="trigger" size="small" type="primary">选取图片</el-button><span style="margin-left:20px">图片尺寸（1900*618px）</span>
            </el-upload>
...
export default {
    ...
    methods:{
        ...
        // 新增一条的接口
        addServer(pamas){
            postRequest(url.carouselInsertCarousel, pamas).then(data=> {
                this.uploadNum=0
        ...
        this.$confirm('是否删除?', '提示', {
            ...
            type: 'warning'
                ...
                postRequest(url.carouselDeleteCarouselById, {
                    id:rows[index].id,
                    url:rows[index].url,
                }).then(data=> {
        ...
        initData(pamas){
            postRequest(url.carouselSelctCarouselList, pamas).then(data=> {
                if(!getErr(data.data.code,data.data.message)) {
                    return
                }
